...
class Provided
{
    ...
    public function index($table)
    {
        ...
        $sql = "SELECT providedes.*, categories.category,media.m_name FROM `providedes` JOIN media ON providedes.media_id = media.id JOIN categories ON providedes.cat_id = categories.id ORDER BY id DESC ";
    ...
    //    exit;
        ...
        if ($cause == '' && $collect_amt == '' && $description == '' && $data == ''   ) {
            ...
            try{
                $sql = "INSERT INTO providedes (cause,provid_amt,date,description,media_id,cat_id)VALUES(:cause,:provid_amt,:date,:description,:media_id,:cat_id)";
                $query = $this->db->pdo->prepare($sql);
                $query->bindValue('cause', str_replace(' ', '_', trim($cause)));
                $query->bindValue('provid_amt', str_replace(' ', '_', trim($provid_amt)));
                ...
                if ($query->execute() == 1) {
                    $result = "Provid Amount Added successfully !";
                }
                return $result;
    ...
    {
        $sql = " SELECT * FROM providedes 
        JOIN media ON providedes.media_id = media.id 
        JOIN  categories ON providedes.cat_id = categories.id 
        WHERE providedes.media_id =".$id; 
        $query = $this->db->pdo->prepare($sql);
        $query->execute();
        $results = $query->fetchAll();
        return $results;
